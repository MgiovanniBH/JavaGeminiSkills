...
* **Validation:** Before marking any task, refactoring, or code modification as complete, the agent **MUST** execute the following verification sequence:
  ...
  3. **Run Tests:** Run the complete local test suite using the Gradle wrapper (`./gradlew.bat test`) to guarantee absolute system stability and regression safety.