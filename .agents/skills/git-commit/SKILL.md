...
### Subject Rules
- Idioma: **Português do Brasil (pt-BR)** é obrigatório (ex: "refactor: aplicar code review", "feat: adicionar nova api").
- Imperative mood: "Adiciona suporte" não "Adicionado suporte"
- No period at end
- Max 50 chars
...
- O corpo também deve ser escrito em Português do Brasil.
- Wrap at 72 chars
...
1. **Validate changes**: ALWAYS run `./gradlew clean compileJava test` before committing.
2. **Analyze results**: Only proceed if the build and all tests pass.
3. **Analyze changes** using `git diff --staged`
4. **Identify scope** from modified files
5. **Determine type** based on change nature
6. **Generate message** following format
